...
MOVEMENT_MIN = 30  # Original: 20
MOVEMENT_MAX = 36  # Original: 30
IDLE_MIN = 2  # Original: 0
IDLE_MAX = 6  # Original: 10
...
# Must be the same value as in render_videos.py.
_SCENE_EXTRA_FRAMES = 32
...
def random_objects_movements(
        ...
        max_motions=MAX_MOTIONS, allow_contain=True, snitch_move_prob=None):
    ...
    all_obj_locations = [
        list(itertools.repeat(obj.location.copy(), args.num_frames + _SCENE_EXTRA_FRAMES + 1))
    ...
    # Cut_objects.
    # cut_obj_locations = zip(*random.sample(list(zip(objects, all_obj_locations)), args.non_static_objects))
    # Now start by time intervals, and add a single object or multiple object
    # action sequence.
    cur_frame = 0
    seed = random.randint(0, 1000000)  # For consistent multi-view.

    while cur_frame <= total_frames:  # - MOVEMENT_MAX:
        print(f'Adding object motion for frame {cur_frame}...')
        ops = [add_movements_multiObj_try, add_movements_singleObj]
        if not args.singular_movement:
        ...
        for attempt in range(5):
            try:
                end_frame = op(
                    ...
                    record, seed, max_motions=max_motions, allow_contain=allow_contain,
                    snitch_move_prob=snitch_move_prob)
                break
            except Exception as e:
                # NOTE: infinite loop here?
                if attempt >= 4:
                    raise e
                print('Exception in random_objects_movements! Retrying with add_movements_singleObj...')
                print(e)
                op = ops[1]
...
def add_movements_multiObj_try(objects, start_frame, all_obj_locations,
                               min_dist, total_frames, record, seed,
                               max_motions=MAX_MOTIONS, allow_contain=True,
                               snitch_move_prob=None):
    for _ in range(100):  # try 10 times to find
        ...
        new_start_frame = start_frame + random.randint(IDLE_MIN, IDLE_MAX)
        new_end_frame = min(new_start_frame + frames_this_move, total_frames)
        # new_end_frame = new_start_frame + frames_this_move
        ...
        new_end_frame_singleObjMotion = add_movements_singleObj(
            ...
            start_frame,
            all_obj_locations,
            min_dist, total_frames, record, seed=seed, ignore_obids=[affected_idx],
            max_motions=(max_motions - 1), allow_contain=allow_contain,
            snitch_move_prob=snitch_move_prob)
        # The following should always be true
        for i in range(len(objects)):
            assert len(objects[i]) == len(all_obj_locations[i]), \
...
def add_movements_singleObj(objects, start_frame, all_obj_locations, min_dist,
                            total_frames, record, seed, ignore_obids=(),
                            max_motions=MAX_MOTIONS, allow_contain=True,
                            snitch_move_prob=None):
    # order to iterate through the frames in
    if snitch_move_prob is not None and np.random.rand() < snitch_move_prob:
        # Ensure the first object is handled first.
        obj_order = np.array(
            [0] + list(np.random.RandomState(seed=seed).permutation(len(objects) - 1) + 1))
    else:
        obj_order = np.random.RandomState(seed=seed).permutation(len(objects))
    # Remove any object IDs in ignore_obids. They have either already been
...
def assert_no_collisions(obj_locs, objs, min_dist, record, ignore_obids=()):
    # October 2020 -- random crashes here, let's not check at all.
    # Reverted March 2021.
    return
...
def add_movements(objs, record, start_frame, end_frame,
    ...
    the outermost.
    """
    is_snitch = objs[0][0]['shape'] == 'spl'
    vagabond_snitch = snitch_move_prob is not None
    ...
    elif is_snitch and vagabond_snitch and random.random() < snitch_move_prob:  # snitch
        all_actions = [
            ([_slide], False),
            ([_pick_place], False),
        ]  # no rotate for these
    ...
    while True:  # try to find a movement that does not collide with others
        ...
        if len(intersection(action, [_slide, _pick_place])) > 0:
            if is_snitch and vagabond_snitch:
                old_loc = objs[0][1].location.copy()
                (new_x, new_y) = _random_far_away_xy(
                    old_loc, half_range=3.5, min_dist=3.1, min_angle=60.0)
            else:
                half_range = 3.1  # Before: 3.0.
                new_x = random.uniform(-half_range, half_range)
                new_y = random.uniform(-half_range, half_range)
            kwargs.update(
                {'x': new_x, 'y': new_y})
...
def _random_far_away_xy(old_location, half_range=3.5, min_dist=2.5, min_angle=45.0):
    x = random.uniform(-half_range, half_range)
    y = random.uniform(-half_range, half_range)
    old_theta = math.atan2(old_location[1], old_location[0])
    while random.random() < 0.9999:
        x = random.uniform(-half_range, half_range)
        y = random.uniform(-half_range, half_range)
        theta = math.atan2(y, x)
        angle = min(abs(theta - old_theta), abs(theta - old_theta - np.pi * 2.0),
                    abs(theta - old_theta + np.pi * 2.0))
        angle *= 180 / np.pi
        if (old_location[0] - x) ** 2 + (old_location[1] - y) ** 2 >= min_dist ** 2 and \
                angle >= min_angle:
            break
    return (x, y)
